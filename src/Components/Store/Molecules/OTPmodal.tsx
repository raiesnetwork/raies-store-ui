...
const OtpVerify: React.FC = () => {
    const {setIsOtpModalVisible,modalOpener,verifyEmail,verifyNumber}=useMystoreStore((s)=>s)
  ...
  const handleSubmit = async() => {
    ...
    if (modalOpener) {
     ...
     if (mailResp?.error) {
        ...
        // It's a phone number
      const numberResp=await  updateMobileNumberApi(modalOpener,otp.join(''),hostName);
      if (numberResp?.error) {
        toast.error("OTP verification error")
     }else{
        if (!numberResp?.data) {
            ...
            toast.error("This number is already registered. Please use a different number.");
  ...
  return (
    <div style={styles.modalBackground}>
      <div style={styles.modalContent}>
        <p onClick={()=>setIsOtpModalVisible('')} 
         style={{textAlign:"right",cursor:"pointer"}}>X</p>
        ...
        <button onClick={handleSubmit} style={styles.verifyButton}>Verify</button>
      </div>
    </div>
  );
};
